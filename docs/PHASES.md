...
**Scope:** `apps/miniapp` — the primary shopping surface, a Next.js 15 App Router client hitting the same `/api/v1/*` REST endpoints the bot and any future website use, with zero server-side business logic of its own.

- **Auth**: `TelegramProvider` loads Telegram's classic `telegram-web-app.js` via `next/script`, calls `ready()`/`expand()`, then POSTs `window.Telegram.WebApp.initData` to `POST /auth/telegram` — the same HMAC-verified path (`packages/shared/src/crypto/telegram-init-data.ts`, built in Phase 3) the bot's Mini App button opens. Degrades honestly when not opened inside Telegram: `inTelegram`/`authError` state surfaces a plain "open this app inside Telegram" notice instead of pretending to be signed in.
- **State**: two Zustand stores (`auth-store`, `cart-store`) persisted to `localStorage`. Both use `skipHydration: true` plus a mount-only `StoreHydration` component that calls `.persist.rehydrate()` — server-rendered markup and the client's first paint both start from empty state, then rehydrate a tick after mount, which avoids a React hydration mismatch on every page that reads auth/cart state instead of just risking one. `cart-store.addItem` rejects adding a second currency to a non-empty cart (checkout only ever sends one currency) and keeps a single `idempotencyKey` per checkout attempt, generated once and cleared only after a confirmed order — a double-tap on "Place order" reuses the same key rather than minting a new one, which is what makes Phase 5's `(customerId, idempotencyKey)` unique constraint actually protect this UI.
- **Pages**: `/shop` (categories rail + featured grid), `/categories`, `/products` (search — debounced 300ms — + category filter via query params, wrapped in `Suspense` for `useSearchParams`), `/products/[slug]` (quantity stepper, add-to-cart), `/cart`, `/checkout` (payment-method selection filtered to the cart's currency, calls `POST /orders/checkout`), `/orders`, `/orders/[id]` (status-specific guidance, payment instructions cross-referenced from `GET /payment-methods` by `paymentMethodId`, and the proof-upload widget gated to `PENDING_PAYMENT` exactly like the backend gates it), `/account`. `/support` is an honest placeholder — Phase 10 hasn't built the support module yet, so this renders "coming soon" rather than a UI against endpoints that don't exist.
- Product/category/payment-method images render via a plain `<img>`, not `next/image` — sources are arbitrary admin-configured storage URLs (`S3_PUBLIC_URL`/LocalStack today, anything in production), and `formatMoney` falls back to `"<amount> <code>"` when `Intl.NumberFormat` rejects a currency code, since payment-method/product currencies are admin free text (crypto tickers included), not guaranteed ISO 4217.

**Verification — genuine browser testing, not just typecheck:**
- `pnpm --filter @sqlm/miniapp typecheck` / `lint` / `build` — all clean (12 routes, 10 static + 2 dynamic).
